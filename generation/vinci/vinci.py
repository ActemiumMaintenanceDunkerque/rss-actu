...
from utils.index_rss import index_rss
...
presse_tree = ET.parse("../../rss/vinci/presse.rss")
presse_root = presse_tree.getroot()
presse_file = ET.tostring(presse_root, encoding="unicode")

actu_tree = ET.parse("../../rss/vinci/actu.rss")
actu_root = actu_tree.getroot()
actu_file = ET.tostring(actu_root, encoding="unicode")

presse_index = index_rss("../../rss/vinci/presse.rss")
actu_index = index_rss("../../rss/vinci/actu.rss")
...
def scrap(url: str, category: str, file: str, first_page: bool = False, affichage: bool = False):
    ...
    for article in articles:
        ...
        if article["Titre"] not in presse_file and article["Titre"] not in actu_file:

            # Description générée par IA
            description = scrap_summary(article["Lien"])

            if description != "Erreur de génération du résumé":
            ...
            print("L'article existe déjà et ne sera pas régénéré")

            infos = presse_index.get(article["Titre"]) if article["Titre"] in presse_file else actu_index.get(article["Titre"])
            ...
            if infos:
                ...
                print("ERREUR : titre trouvé dans presse_file ou actu_file mais pas dans le RSS")
